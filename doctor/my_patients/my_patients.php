...
<div class="card">
    <div class="card-header border-bottom">
        <?php
        include ("../../connection.php");
        $squery = mysqli_query($con, "SELECT * FROM appointment a left join patient p on p.id = a.patientid where doctorid = '".$_SESSION['userid']."' GROUP BY patientid ");
        $num_rows = mysqli_num_rows($squery);

         echo '<h5 class="mb-0"> My Patients  ('.$num_rows.') </h5> ';   
         ?>                                       
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-hover table-nowrap" >
            ...
            <?php
                $squery = mysqli_query($con, "SELECT *,CONCAT(p.fname, ' ' ,p.lname) as patientname, CONCAT(scheduleDate, ' ' ,scheduleTime) as DateTime, CONCAT(p.gender) as gender, CONCAT (p.birthDate) as bdate, CONCAT(p.username) as email, CONCAT(p.contactNumber) as contact FROM appointment a left join patient p on p.id = a.patientid where doctorid = '".$_SESSION['userid']."' GROUP BY patientid");
                ...
                <tr>
                    ...
                    <a class="text-heading font-semibold" href="#">'.$row['patientname'].'</a>
